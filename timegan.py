...
def timegan(ori_data, parameters):
  ...
  """
    ...
    early_stop_acc = []
    for itt in range(iterations):
        # if itt > (iterations/2) and (early_stop_acc - .5) < .05:
        #     break
        # else:
        ...
        step_d_loss_list.append(step_d_loss)
        step_g_loss_u_list.append(step_g_loss_u)
        step_g_loss_s_list.append(np.sqrt(step_g_loss_s))
        step_g_loss_v_list.append(step_g_loss_v)
        step_e_loss_t0_list.append(np.sqrt(step_e_loss_t0))
        ...
        if itt % int(iterations / 10) == 0:
            ...
            show = True
        if (itt < (itt/10) and itt % int(iterations / 1000) == 0) or itt % int(iterations / 100) == 0:
            ...
            ori_time_ = [36 for i in range(10000)]
            Z_mb_ = random_generator(10000, z_dim, ori_time_, max_seq_len)
            generated_data_curr_ = sess.run(X_hat, feed_dict={Z: Z_mb_, X: ori_data, T: ori_time_})

            generated_data2 = list()

            for i in range(10000):
                temp = generated_data_curr_[i, :ori_time_[i], :]
                generated_data2.append(temp)

            # Renormalization
            generated_data2 = generated_data2 * max_val
            generated_data2 = generated_data2 + min_val

            score = []
            for _ in range(3):
                score.append(discriminative_score_metrics(ori_data, generated_data, itt=200))
            score = np.mean(score)
            print(score)
            early_stop_acc.append(score)
            visualization(ori_data, generated_data, 'pca', itt=itt, show=show)
            visualization(ori_data, generated_data, 'tsne', itt=itt, show=show)

            np.savez('visualizations/current_run/datasets/timegan_results{}.npz'.format(itt),
                     ori=ori_data, gen=generated_data, gen_long=generated_data2,
                     disc_acc=early_stop_acc)

    print('Finish Joint Training')

    plt.plot(step_d_loss_list, label='step_d_loss')
    plt.plot(step_g_loss_u_list, label='step_g_loss_u')
    plt.legend()
    plt.show()
    plt.plot(step_g_loss_s_list, label='step_g_loss_s')
    plt.legend()
    plt.show()
    plt.plot(step_g_loss_v_list, label='step_g_loss_v')
    plt.legend()
    plt.show()
    plt.plot(step_e_loss_t0_list, label='step_e_loss_t0')
    plt.legend()
    plt.show()

    plot = pd.DataFrame()
    plot['disc_acc'] = early_stop_acc
    plot['rolling'] = plot['disc_acc'].rolling(10).mean()
    ...
    ## Synthetic data generation
    # print(ori_time.shape)
    # print(ori_time)
    # print([24 for i in range(10000)])
    ori_time_ = [36 for i in range(10000)]
    Z_mb_ = random_generator(10000, z_dim, ori_time_, max_seq_len)
    generated_data_curr_ = sess.run(X_hat, feed_dict={Z: Z_mb_, X: ori_data, T: ori_time_})
    ...
    return generated_data, generated_data2
